...
internals.Auth.prototype.scheme = function (name, scheme) {
    ...
    Hoek.assert(name, 'AuthenticationServices scheme must have a name');
    Hoek.assert(!this._schemes[name], 'AuthenticationServices scheme name already exists:', name);
...
internals.Auth.prototype.strategy = function (name, scheme /*, mode, options */) {
    ...
    Hoek.assert(name, 'AuthenticationServices strategy must have a name');
    Hoek.assert(name !== 'bypass', 'Cannot use reserved strategy name: bypass');
    Hoek.assert(!this._strategies[name], 'AuthenticationServices strategy name already exists');
    Hoek.assert(scheme, 'AuthenticationServices strategy', name, 'missing scheme');
    Hoek.assert(this._schemes[scheme], 'AuthenticationServices strategy', name, 'uses unknown scheme:', scheme);
...
internals.Authenticator = class {
    ...
    validate(err, result, next) {                 // err can be Boom, Error, or a valid response object
        ...
        if (!err &&
            ...
            return next(Boom.badImplementation('AuthenticationServices response missing both error and credentials'));
        }
